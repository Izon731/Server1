--[[
addons/hl2rp_weapons_base/lua/weapons/swb_base/cl_hud.lua
--]]
...
function SWEP:DrawHUD()
	...
	draw.ShadowText(self.FireModeDisplay, "SWB_HUD16", x - sc1 - self.BulletDisplay * 1, y - sc2 - 4, White, Black, 1, TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER)
		
	if self.BulletDisplay and self.BulletDisplay > 0 then
		surface.SetTexture(Bullet)
		for i = 1, self.BulletDisplay do
			surface.SetDrawColor(0, 0, 0, 255)
			surface.DrawTexturedRect(x - sc1 - (i - 1) * 20 - 8, y - sc2 - 0, 16, 16)
			surface.SetDrawColor(255, 255, 255, 255)
			surface.DrawTexturedRect(x - sc1 - (i - 1) * 20 - 8, y - sc2 - 0, 16, 16)
		end
	end
